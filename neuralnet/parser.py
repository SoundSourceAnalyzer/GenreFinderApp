...
class FeatureExtractor:
    ...
    def export_features(self):
        ...
        self.fv.extend(filter(lambda a: a != 0, self.ase_per_band_avg)[:31])
        self.fv.append(self.ase_avg)
        self.fv.extend(filter(lambda a: a != 0, self.ase_per_band_var)[:31])
        ...
        self.fv.extend(self.sfm_per_band_avg[:24])
        self.fv.append(self.sfm_avg)
        self.fv.extend(self.sfm_per_band_var[:24])
    ...
    def extract_mpeg7_features(self):
        ...
        mpeg7Path = os.path.join(CURRENT_DIR, 'parser/MPEG7AudioEnc.jar')
        mpeg7ConfigPath = os.path.join(CURRENT_DIR, 'parser/mpeg7config.xml')

        result = subprocess.check_output(['java', '-jar', mpeg7Path, self.audiofile, mpeg7ConfigPath])
    ...
    def extract_mfcc(self):
        fp = FeaturePlan(sample_rate=22050, normalize=1, resample=True)
